...
const prefixFormat = winston_1.default.format((info) => {
...
const websocketFormatFn = winston_1.default.format((info) => {
    ...
    try {
        ...
        const wsController = (0, WebSocketController_1.getWebSocketController)();
...
websocketFormat.sentMessages = new Map();
// Définir le format pour les logs de console
const consoleFormat = winston_1.default.format.combine(winston_1.default.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss:ms' }), winston_1.default.format.colorize({ all: true }), prefixFormat(), 
// N'ajouter le websocketFormat que pour l'API principale, pas pour les workers ni pour les logs liés au WebSocket
...(isMainApiProcess ? [websocketFormat] : []), customFormat);
// Définir le format pour les logs de fichier (sans WebSocket pour éviter la duplication)
const fileFormat = winston_1.default.format.combine(winston_1.default.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss:ms' }), prefixFormat(), winston_1.default.format.json());
